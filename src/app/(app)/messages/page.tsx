...
export default function MessagesPage() {
  ...
  const createCallNotification = useCallback(async (recipientId: string, isVideoCall: boolean, conversationId: string) => {
    if (!currentUser || !recipientId || recipientId === currentUser.uid) {
      console.log('❌ Cannot create call notification:', { currentUser: !!currentUser, recipientId, currentUserId: currentUser?.uid });
      return;
    }

    console.log('📞 Creating call notification:', { recipientId, isVideoCall, conversationId });
    ...
    try {
      ...
      console.log('✅ Call notification created for user:', recipientId, 'with ID:', docRef.id);
    } catch (error) {
  ...
  const markCallAsAnswered = useCallback(async (conversationId: string, callerId: string) => {
    ...
    try {
      ...
      console.log('✅ Call answered notification sent to caller');
    } catch (error) {
  ...
  const markCallAsDeclined = useCallback(async (conversationId: string, callerId: string) => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        console.log('✅ Call marked as declined');
      }
      ...
      console.error('❌ Error marking call as declined:', error);
    }
  }, [currentUser]);
