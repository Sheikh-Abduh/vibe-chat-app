...
import { LogOut, UserCircle, Search, Edit3, Heart, Info, Gift, PersonStanding, Hash, Sparkles, BellDot, MessageSquare } from 'lucide-react';
...
function AppLayoutContent({ children }: { children: React.ReactNode }) {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        setUserStoredDetails({
          ...
          vibeTag: fetchedProfileDetails.vibeTag,
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const unsubscribeFirestore = onSnapshot(q, (querySnapshot) => {
        ...
        console.log('🔔 Layout: Unread activity count updated:', unreadCount);
  ...
  // Listen for activity page visits to immediately clear badge
  useEffect(() => {
    const handleActivityPageVisit = () => {
      console.log('🔔 Layout: Activity page visited, clearing badge immediately');
      setUnreadActivityCount(0);
    };

    // Listen for custom event from activity page
    window.addEventListener('activityPageVisited', handleActivityPageVisit);

    return () => {
      window.removeEventListener('activityPageVisited', handleActivityPageVisit);
    };
  }, []);
